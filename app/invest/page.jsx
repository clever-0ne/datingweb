    ...
    <>
      ...
      <div className="panel relative mb-6 p-6 lg:p-8">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
          <div className="mb-4 lg:mb-0 lg:flex-1">
            <p className="mb-2 text-[11px] font-semibold uppercase tracking-widest mut">Investing</p>
            <h1 className="mb-1 text-2xl font-semibold hi">Investment Plans</h1>
          ...
          <div className="card lg:w-80">
            <div className="mb-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-semibold hi">{INVESTMENT_PLANS.length}</p>
              </div>
              <div className="chip chip-b h-10 w-10 rounded-xl"><TrendingUp size={20} /></div>
            </div>
            <div className="flex gap-4 text-xs">
              <div className="flex-1 text-center"><p className="mut">Term</p><p className="font-semibold hi">{TERM_LABEL}</p></div>
              <div className="flex-1 text-center"><p className="mut">Return</p><p className="font-semibold hi">{MULTIPLE}</p></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        {PLATFORM.map((p) => (
          <div key={p.label} className="panel p-5">
            ...
            <p className={`mt-2 text-2xl font-bold ${p.green ? 'grn' : 'hi'}`}>{p.value}</p>
      ...
      <div className="mb-8">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-light hi">Investment Plans</h2>
              ...
              <div key={p.id} className={`card card-glow ${tierGlow(p.name, idx)} flex flex-col p-6 ${plan === p.id ? 'card-sel' : ''}`}>
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-medium hi">{p.name}</h3>
                ...
                <div className="mt-auto flex items-center justify-between gap-3">
                  <button className="text-xs font-medium hi hover:opacity-70" onClick={() => { setPlan(p.id); setNotice(null); }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="card card-static relative z-10 w-full max-w-md p-6">
            <div className="mb-4 flex items-start justify-between">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-widest mut">Plan Preview</p>
                <h2 className="text-xl font-bold hi">{selPlan.name} · {selPlan.roi}% ROI</h2>
              </div>
              <button onClick={() => { setPlan(null); setNotice(null); }} className="mut hover-tx" aria-label="Close">
                <X size={20} />
              </button>
            </div>

            <div className="mb-5 space-y-2 text-sm">
              <div className="flex justify-between"><span className="mut">Principal</span><span className="font-medium hi">{fmtMoney(selPlan.amount)}</span></div>
              ...
              <div className="flex justify-between"><span className="mut">Term</span><span className="font-medium hi">{selPlan.termDays} days</span></div>
            </div>
            ...
            {insufficient && <p className="mt-2 text-center text-xs text-[var(--bad-text)]">You need {fmtMoney(selPlan.amount - balance)} more in your account balance.</p>}
          </div>
        </div>
      )}
    </>
...
function Row({ k, v, green, purple }) {
  return (
    <div className="flex justify-between">
      ...
      <span className={`text-sm font-medium ${green ? 'grn' : purple ? 'tert' : 'hi'}`}>{v}</span>
    </div>
  );
}
