...
export default function PaymentCheckout({ kind = 'deposit', context = 'Deposit', defaultAmount = 500, backHref = '/deposit' }) {
  ...
  return (
    <>
      {/* Header */}
      <div className="relative mb-6 overflow-hidden rounded-3xl p-6 sm:p-8" style={{ background: 'linear-gradient(120deg,rgba(10,84,255,.18),var(--ok-bg))', border: '1px solid var(--hairline-strong)' }}>
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold hi sm:text-3xl">Make Payment</h1>
          ...
          <div className="flex items-center gap-3 rounded-2xl border p-3" style={{ background: 'var(--soft)', borderColor: 'var(--hairline-strong)' }}>
            <div className="flex h-12 w-12 items-center justify-center rounded-xl text-white" style={{ background: 'linear-gradient(135deg, var(--primary-2), var(--secondary))' }}>
              <Icon size={20} />
            </div>
            <div>
              <p className="text-xs mut">Payment Amount</p>
              <p className="text-xl font-bold hi">{fmt(amount)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {done ? (
        <div className="panel mx-auto max-w-md p-8 text-center">
          ...
          <h3 className="mb-2 text-xl font-bold hi">Payment Submitted</h3>
        ...
        <div className="panel mx-auto max-w-4xl overflow-hidden">
          ...
          <div className="flex items-center gap-3 border-b p-6" style={{ borderColor: 'var(--soft)' }}>
            ...
            <div>
              ...
              <p className="text-lg font-semibold hi">{coin.name} · {context}</p>
            </div>
          </div>

          <div className="p-6">
            {/* Coin selector + amount */}
            <div className="mb-6 rounded-xl border p-4 text-center" style={{ borderColor: 'var(--hairline)', background: 'var(--soft)' }}>
              <p className="hi">You are to make payment of <span className="font-bold">{fmt(amount)}</span> using your selected method.</p>
            </div>
            ...
            <div className="mb-6 grid grid-cols-2 gap-2 sm:grid-cols-4">
              {coins.map((c) => (
                <button
                  ...
                  className={`rounded-xl border p-3 text-center transition ${coinId === c.id ? 'border-[color:var(--primary)] bg-[rgba(10,84,255,.15)]' : 'border-[var(--soft-2)] hover:border-[var(--hairline-strong)]'}`}
                >
            ...
            {/* Address */}
            <div className="mb-4">
              <h3 className="mb-2 text-lg font-semibold hi">{coin.name} Address:</h3>
              <div className="flex items-center justify-between gap-2 rounded-xl border border-dashed px-3 py-2.5" style={{ borderColor: 'var(--hairline-strong)' }}>
              ...
              <p className="mt-1 text-sm mut"><span className="font-semibold hi">Network Type:</span> {coin.network}</p>
            </div>

            {/* QR placeholder */}
            <div className="mb-6 flex justify-center">
              <div className="relative rounded-xl border-2 p-4" style={{ borderColor: 'rgba(240,185,11,.5)' }}>
                <div className="absolute -right-3 -top-3 flex h-8 w-8 items-center justify-center rounded-full hi" style={{ background: 'var(--accent)' }}>
                  <QrCode size={16} />
                </div>
                <div className="flex h-[200px] w-[200px] items-center justify-center rounded-lg" style={{ background: 'var(--soft)' }}>
                  <QrCode size={120} className="faint" />
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border p-5" style={{ borderColor: 'var(--hairline)' }}>
              <h4 className="mb-4 font-semibold hi">How to Complete Your Payment</h4>
