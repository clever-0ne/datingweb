...
const ARRAY_DEFAULTS = new Set([
  ...
  'miningWithdrawals', 'investmentWithdrawals', 'notifications', 'boosts',
  'pushSubscriptions', 'activity', 'accounts',
]);
...
function upgradeAdmin(state) {
  return hashLegacyAdmin(state) || syncAdminPassword(state);
}
...
export const readDb = cache(async function readDb() {
  ...
  for (const k of COLLECTIONS) {
    ...
    writes.push(
      sql()`
        ...
        VALUES (${col(k)}, ${serialised}::jsonb, now())
